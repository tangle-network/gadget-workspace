...
async fn test_eigenlayer_incredible_squaring_blueprint() {
    ...
    // let harness = EigenlayerTestHarness::setup().await.unwrap();
    ...
    // let response_listener_address = setup_task_response_listener(&harness, task_manager_address, successful_responses.clone()).await;
    ...
    //     provider,
    // );
    // let initialize_task = InitializeBlsTaskEventHandler::new(contract.clone(), aggregator_context.clone());
